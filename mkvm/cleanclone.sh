...
hosts=("ctlnode" "node-0" "node-1")


if ! vboxmanage snapshot base list|grep -q 'Name'; then
  echo Creating Snapshot $snapshot
  vboxmanage snapshot ${basevm} take $snapshot --description="preparing to make a clone from cli"
fi

for var in "${!hosts[@]}"; do
  echo poweroff ${hosts[var]}
  vboxmanage controlvm ${hosts[var]} poweroff
    sleep 2
  echo unregister ${hosts[var]}
  vboxmanage unregistervm ${hosts[var]} --delete
    sleep 2
